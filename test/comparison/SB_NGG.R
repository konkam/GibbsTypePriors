...
density_xi_n<- function(x, alpha, b, theta, k_n,n){
  ...
  term2 = (b/kn + x)^(alpha - 1)
  term3 = x^(theta + (n-1)*alpha)
  return (term1*term2*term3)
}

xi<- vector("numeric", length=25000)
for(i in 1:25000){
...
lines(x,density_xi_n(x,0.75,1,1,0.5,1)*2.62)
...
#p_ngg <- Prior_on_K_SB_NGG(0.25, 39,1, 100,300, runs=10^3)
